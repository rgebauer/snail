...
public class Avoidance : MonoBehaviour {
	...
	void Update () 
	{
		...
		if (Physics.Raycast(transform.position, new Vector3(-distance, 0, 0), out hit, distance))
		{
			...
			{
				if (DebugMode)
					UnityEngine.Debug.DrawLine(transform.position, transform.position + new Vector3(-distance, 0, 0));
		...
		if (Physics.Raycast(transform.position, new Vector3(0, 0, distance), out hit, distance))
		{
			...
			{
				if (DebugMode)
					UnityEngine.Debug.DrawLine(transform.position, transform.position + new Vector3(0, 0, distance));
				float resistPower = 0.1f - hit.distance/(distance*10);
		...
		if (Physics.Raycast(transform.position, new Vector3(distance, 0, distance), out hit, distance))
		{
			...
			{
				if (DebugMode)
					UnityEngine.Debug.DrawLine(transform.position, transform.position + new Vector3(distance, 0, distance));
				float resistPower = 0.1f - hit.distance/(distance*10);
